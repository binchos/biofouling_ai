...
class LiaciDataset(Dataset):
    ...
    data/liaci/
      ...
      splits.csv: id,split
    """
    def __init__(self, root="/home/shared_project/biofouling_ai/data/kowp/kowp_prepared/images_near", split="train", transform=None, strict=True, size: Optional[Tuple[int,int]]=None):
        self.root = Path(root)
        self.split = split
        self.meta = pd.read_csv(self.root / "splits_26.csv")
        self.meta = self.meta[self.meta["split"] == split].reset_index(drop=True)
        self.transform = transform
        self.strict = strict
        self.size = size
